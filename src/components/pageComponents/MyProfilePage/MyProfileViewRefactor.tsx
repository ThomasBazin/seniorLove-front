  ...
  const handleSubmit = async () => {
    try {
      ...
      const dataToSend = {
        ...
        birth_date: editedProfile.age
          ? new Date(
            new Date().setFullYear(
              new Date().getFullYear() - parseInt(editedProfile.age as string)
            )
          ).toISOString()
          : undefined,
            ...
            <div>
              ...
              {isEditing ? (
                <input
                  type="number"
                  name="age"
                  value={editedProfile.age || calculateAge(me.birth_date)}
                  onChange={handleInputChange}
                  className="text-black w-16 px-2 py-1 rounded"
                />
              ) : (
                calculateAge(me.birth_date)
              )}{' '}
              ans
            </div>
            <div className="pt-4">
